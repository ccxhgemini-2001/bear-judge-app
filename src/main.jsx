...
/**
 * --- 生产环境终极配置 ---
 * 熊在这里使用了最原始、最稳固的读取方式，确保 Vercel 和手机浏览器都能识别。
 */
const safeParse = (val) => {
  if (!val) return null;
...
const firebaseConfig = safeParse(rawConfig);
const apiKey = (typeof window !== 'undefined' && window.__api_key) ? window.__api_key : import.meta.env.VITE_GEMINI_API_KEY;
...
if (firebaseConfig && firebaseConfig.apiKey) {
  try {
    ...
    auth = getAuth(app);
    db = getFirestore(app);
  } catch (e) { console.error("Firebase Init Crash:", e); }
}
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        setError("法庭内勤认证失败：请确认 Firebase 的 Anonymous 登录已开启嗷。");
      } finally {
        setInitializing(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(caseDoc, (snap) => {
      ...
      setError("调取卷宗失败：权限不足或路径错误，请检查 Firebase Rules。");
    });
    return () => unsubscribe();
  }, [user, caseId]);
  ...
  const createCase = async (chosenRole) => {
    ...
    try {
      ...
      setCaseId(newId);
    } catch (err) { setError("卷宗归档失败，请检查数据库 Rules。"); }
  ...
  const triggerAIJudge = async () => {
    if (!currentCase || !apiKey) { setError("AI 宣判大脑未联网。"); return; }
    setLoading(true);
    setError("");
    const systemPrompt = `你是一位名为“轻松熊法官”的AI情感专家。这里是轻松熊王国神圣最高法庭。语气严肃专业且治愈，自称“熊”。仅输出严格JSON，不要Markdown。
  ...
  if (initializing) {
    return (
      <div className="min-h-screen bg-[#FFFDFB] flex flex-col items-center justify-center text-[#8D6E63] p-10 text-center">
        ...
        <p className="text-xs text-[#A1887F]">如果长时间停留在此页面，请检查 Vercel 环境变量设置嗷～</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FFFDFB] text-[#4E342E] font-sans pb-10 select-none overflow-x-hidden text-balance">
      <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-20 p-4 border-b border-[#F5EBE0] flex justify-between items-center px-6 shadow-sm">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => {setClickCount(prev => prev + 1); if(clickCount >= 4) {setDevMode(!devMode); setClickCount(0);}}}>
      ...
      <div className="max-w-xl mx-auto p-4 pt-6">
        <div className="relative mb-8 rounded-[2.5rem] shadow-2xl overflow-hidden border-[6px] border-white aspect-[16/9] bg-[#F5EBE0]">
          ...
          <div className="absolute bottom-6 left-8 flex items-end justify-between right-8">
            <h1 className="text-white font-black text-2xl drop-shadow-lg leading-none font-bold">公正 · 治愈 · 爱</h1>
            <Landmark className="text-white/60 mb-1" size={36} />
          </div>
        </div>

        {!caseId ? (
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-[#F5EBE0] text-center animate-in fade-in zoom-in-95 duration-500 relative">
            ...
            <p className="text-[#8D6E63] text-sm mb-12 px-6 font-medium leading-relaxed">这里是王国最神圣的地方嗷，熊将抱着极其认真的心情，帮你们化解每一颗受委屈的小心心。</p>
              ...
              {showRoleSelect ? (
                ...
                <><button onClick={() => setShowRoleSelect(true)} className="w-full bg-[#8D6E63] text-white py-5 rounded-[2rem] font-black text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"><UserPlus size={22}/> 发起新诉讼</button>
                  <div className="flex gap-2 mt-4 items-stretch">
                    <input placeholder="输入卷宗检索码" className="flex-1 min-w-0 p-5 rounded-[1.8rem] bg-[#FDF5E6] border-2 border-transparent focus:border-amber-200 outline-none text-center font-black tracking-widest uppercase text-sm" onChange={(e) => setTempInput(e.target.value)} />
                    <button onClick={() => joinCase(tempInput)} className="flex-shrink-0 bg-white border-2 border-[#8D6E63] text-[#8D6E63] px-6 rounded-[1.8rem] font-black active:bg-[#FDF5E6] transition-colors shadow-sm text-sm">调取</button>
                  </div></>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white p-6 rounded-[2.5rem] flex justify-between items-center shadow-md border border-[#F5EBE0]">
              ...
              <button onClick={() => navigator.clipboard.writeText(caseId)} className="p-3 bg-[#FDF5E6] text-[#8D6E63] rounded-2xl hover:bg-[#F5EBE0] transition-colors"><Copy size={20} /></button>
            </div>
              ...
              <div className="animate-in slide-in-from-bottom-20 duration-1000 pb-10">
                <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl border-t-[14px] border-[#8D6E63] relative overflow-hidden">
                  ...
                  <div className="mb-14 bg-[#FDFBF9] p-8 rounded-[2.5rem] border border-[#F5EBE0] shadow-inner">
                    <div className="flex justify-between mb-5 text-[11px] font-black uppercase tracking-widest"><span className="text-blue-600 font-bold">男方归因 {verdictData.fault_ratio?.A || 50}%</span><span className="text-rose-600 font-bold">女方归因 {verdictData.fault_ratio?.B || 50}%</span></div>
                    <div className="w-full h-5 bg-gray-100 rounded-full overflow-hidden flex shadow-inner border border-gray-200"><div className="h-full bg-gradient-to-r from-blue-300 to-blue-400 transition-all duration-1000" style={{ width: `${verdictData.fault_ratio?.A || 50}%` }} /><div className="h-full bg-gradient-to-r from-rose-300 to-rose-400 transition-all duration-1000" style={{ width: `${verdictData.fault_ratio?.B || 50}%` }} /></div>
                  </div>
                  ...
                  <div className="mt-16 pt-12 border-t-4 border-double border-[#F5EBE0]">
                    ...
                    <div className="grid grid-cols-1 gap-4">{(verdictData.punishments || []).map((p, i) => (<div key={i} className="bg-white border-2 border-[#F5EBE0] p-6 rounded-[2rem] text-center text-sm font-black shadow-sm transition-all active:border-amber-300">{String(p)}</div>))}</div>
                  </div>
