...
/**
 * --- 生产环境标准配置 (Vercel 专供) ---
 * 熊提示：此处必须使用静态字面量读取，否则 Vite 会在构建时将其忽略。
 * 请无视预览窗口的 import.meta 警告，这是为了确保手机端不白屏的唯一方案。
 */
const safeParse = (val) => {
  if (!val) return null;
...
// --- 配置注入区 ---
// 1. 优先读取 Canvas 全局变量，否则读取 Vite 环境字面量
...
const App = () => {
  ...
  useEffect(() => {
    if (!auth) {
      setError("熊发现配置没读到嗷！请确认 Vercel 里的变量名是 VITE_FIREBASE_CONFIG 且记得 Redeploy 嗷！");
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      setUser(u);
      if (u) {
        setInitializing(false); // 关键：只有拿到用户身份才放行
      }
    });
    ...
    const initAuth = async () => {
      try {
        ...
        setError("身份认证同步失败：请去 Firebase 检查 Anonymous 登录开了没嗷。");
        setInitializing(false);
      }
    };
  ...
  const triggerAIJudge = async () => {
    if (!currentCase || !apiKey) {
      setError("AI 宣判大脑连接异常，请检查 VITE_GEMINI_API_KEY。");
      return;
    }
    setLoading(true);
    setError("");
        ...
        {!caseId ? (
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white p-6 rounded-[2.5rem] flex justify-between items-center shadow-md border border-[#F5EBE0]">
              ...
              <button onClick={() => navigator.clipboard.writeText(caseId)} className="p-3 bg-[#FDF5E6] text-[#8D6E63] rounded-2xl hover:bg-[#F5EBE0]"><Copy size={20} /></button>
            </div>
            {!verdictData ? (
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-[#F5EBE0] min-h-[400px] flex flex-col relative overflow-hidden">
                {isMyTurn ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center py-12 animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-[#8D6E63] text-xs mb-12 px-10 font-medium leading-relaxed">{isBothSubmitted ? '双方证词均已归入法典。点击下方按钮，熊要宣判了嗷！' : '熊还在等待另一半提交内心辩词嗷。'}</p>
              ...
              <div className="animate-in slide-in-from-bottom-20 duration-1000 pb-10">
                <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl border-t-[14px] border-[#8D6E63] relative overflow-hidden">
                  ...
                  <div className="mt-16 pt-12 border-t-4 border-double border-[#F5EBE0]"><h3 className="text-center font-black text-[#8D6E63] text-2xl mb-10 uppercase tracking-widest leading-none">和好罚单执行</h3><div className="grid grid-cols-1 gap-4">{(verdictData.punishments || []).map((p, i) => (<div key={i} className="bg-white border-2 border-[#F5EBE0] p-6 rounded-[2rem] text-center text-sm font-black shadow-sm transition-all active:border-amber-300">{String(p)}</div>))}</div></div>
