 ...
 * 【构建工程师指令：禁止修改 import.meta.env 的字面量写法】
 * Vite 构建生产环境时，必须扫描到这些特定字符串才能执行真实密钥替换。
 * 预览窗口报错是由于其编译环境 es2015 不支持 ESM，不影响 Vercel 部署后的实际运行。
 */
...
const App = () => {
  ...
  const checkFoundation = () => {
    ...
    console.log("------------------------");
    setError(`自检完成！API 状态：${apiKey ? '就绪' : '缺失'}。请查看控制台 F12 日志。`);
  };
  ...
  const triggerAIJudge = async () => {
    ...
    if (!apiKey) { 
      setError("AI 宣判核心无法启动：API 密钥未注入。请务必执行 Vercel Redeploy 重新构建。"); 
      return; 
    }
    ...
    const systemPrompt = `你是一位名为“轻松熊法官”的AI情感专家。必须输出严格 JSON。包含判决标题、归因、诊断、建议等。`;
        ...
        {!caseId ? (
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-[#F5EBE0] text-center animate-in fade-in zoom-in-95 duration-500 relative overflow-hidden">
            ...
            <div className="relative text-balance">
              ...
              <p className="text-[#8D6E63] text-sm mb-12 px-6 font-medium leading-relaxed text-balance text-balance">这里是王国最神圣的地方嗷，熊将帮你们化解委屈。</p>
              <div className="space-y-4">
                {showRoleSelect ? (
                  ...
                  <><button onClick={() => setShowRoleSelect(true)} className="w-full bg-[#8D6E63] text-white py-5 rounded-[2rem] font-black text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 tracking-wide font-bold font-bold"><UserPlus size={24}/> 发起新诉讼</button>
                    <div className="flex gap-2 mt-4 items-stretch h-14">
                      ...
                      <button onClick={() => joinCase(tempInput)} className="flex-shrink-0 bg-white border-2 border-[#8D6E63] text-[#8D6E63] px-6 rounded-[1.5rem] font-black active:bg-[#FDF5E6] text-sm shadow-sm transition-colors tracking-widest tracking-widest font-bold">调取</button>
                    </div></>
                )}
              </div>
            </div>
            ...
            {!currentCase ? (
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-[#F5EBE0] min-h-[400px] flex flex-col relative overflow-hidden text-balance">
                {!userRole && !devMode ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500">
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full px-6">
                       <button onClick={() => pickRoleInCase('male')} disabled={!!currentCase.sideA.uid} className={`p-6 rounded-[2rem] border-2 transition-all shadow-sm ${currentCase.sideA.uid ? 'bg-gray-50 border-gray-100 opacity-40 grayscale text-gray-400 cursor-not-allowed' : 'bg-blue-50 border-blue-100 text-blue-700 active:scale-95'}`}>
                          ...
                          <span className="text-[10px] font-black uppercase tracking-tight">{currentCase.sideA.uid ? '男方已占位' : '我是男方'}</span>
                       </button>
                       <button onClick={() => pickRoleInCase('female')} disabled={!!currentCase.sideB.uid} className={`p-6 rounded-[2rem] border-2 transition-all shadow-sm ${currentCase.sideB.uid ? 'bg-gray-50 border-gray-100 opacity-40 grayscale text-gray-400 cursor-not-allowed' : 'bg-rose-50 border-rose-100 text-rose-700 active:scale-95'}`}>
                          <span className="text-3xl block mb-1">🙋‍♀️</span>
                          <span className="text-[10px] font-black uppercase tracking-tight">{currentCase.sideB.uid ? '女方已占位' : '我是女方'}</span>
                       </button>
                    </div>
                  </div>
                ) : isMyTurn ? (
                  <div className="h-full flex flex-col animate-in slide-in-from-right-4 duration-500">
                    ...
                    <button onClick={submitPart} disabled={loading} className="w-full bg-[#8D6E63] text-white py-5 rounded-[1.8rem] font-black text-xl shadow-lg active:scale-95 transition-all font-bold tracking-widest uppercase font-bold">确认归档</button>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col items-center justify-center text-center py-12 animate-in zoom-in-95 duration-500 text-balance">
                    ...
                    <p className="text-[#8D6E63] text-xs mb-10 px-10 leading-relaxed leading-relaxed text-balance">{isBothSubmitted ? '双方证词均已归入法典。点击按钮启动宣判嗷！' : '正在等待对方提交内心辩词嗷。法庭秩序重于一切～'}</p>
                    <div className="grid grid-cols-2 gap-4 mb-10 w-full px-6">
                      <div className={`p-4 rounded-3xl border flex flex-col items-center gap-1 transition-all duration-500 ${currentCase?.sideA?.submitted ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-gray-50 border-gray-100 text-gray-400 opacity-60'}`}>
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tighter font-bold font-bold">男方证词{currentCase?.sideA?.submitted ? '已就绪' : '待录入'}</span>
                      </div>
                      <div className={`p-4 rounded-3xl border flex flex-col items-center gap-1 transition-all duration-500 ${currentCase?.sideB?.submitted ? 'bg-rose-50 border-rose-100 text-rose-600' : 'bg-gray-50 border-gray-100 text-gray-400 opacity-60'}`}>
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tighter font-bold font-bold">女方证词{currentCase?.sideB?.submitted ? '已就绪' : '待录入'}</span>
                      </div>
                    </div>
                    {isBothSubmitted && (
              ...
              <div className="animate-in slide-in-from-bottom-20 duration-1000 pb-10 text-balance">
                <div className="bg-white rounded-[3.5rem] p-10 shadow-2xl border-t-[14px] border-[#8D6E63] relative overflow-hidden">
                  ...
                  <div className="space-y-10 text-balance text-balance">
                    ...
                    <div className="bg-emerald-50/70 p-8 rounded-[3rem] border border-emerald-100/50 shadow-sm relative text-balance text-balance text-balance"><h4 className="font-black text-emerald-800 mb-4 flex items-center gap-2 italic text-lg font-bold font-bold font-bold font-bold"><Heart size={22} className="text-emerald-500" /> 将心比心 · 懂你才可爱</h4><p className="text-[13px] leading-relaxed text-emerald-900/80 font-medium whitespace-pre-wrap">{String(verdictData.perspective_taking)}</p></div>
                    <div className="bg-indigo-50/50 p-8 rounded-[2.5rem] text-center italic text-sm text-indigo-900/70 font-black leading-relaxed font-bold font-bold">“{String(verdictData.bear_wisdom)}”</div>
                  </div>
                  <div className="mt-16 pt-12 border-t-4 border-double border-[#F5EBE0]">
                    <h3 className="text-center font-black text-[#8D6E63] text-2xl mb-10 uppercase tracking-widest leading-none font-bold tracking-widest font-bold font-bold">和好罚单执行</h3>
                    <div className="grid grid-cols-1 gap-4 font-bold font-bold">
                      {(verdictData.punishments || []).map((p, i) => (
                        <div key={i} className="bg-white border-2 border-[#F5EBE0] p-6 rounded-[2rem] text-center text-sm font-black shadow-sm transition-all hover:translate-y-[-2px] active:border-amber-300 font-bold font-bold">{String(p)}</div>
                      ))}
                    </div>
                  </div>
                  <button onClick={() => {setCaseId(''); setCurrentCase(null); setError("");}} className="w-full mt-14 py-6 text-[#A1887F] text-[11px] font-black tracking-[0.6em] border-t border-[#F5EBE0] pt-10 uppercase active:text-[#8D6E63] font-bold text-sm tracking-widest uppercase font-bold font-bold font-bold">结案 · 拥抱离场</button>
                </div>
              </div>
            )}
          </div>
...
if (typeof document !== 'undefined') {
  ...
  if (isVercelProd) {
    if (!window.__BEAR_JUDGE_GLOBAL_ROOT__) {
       window.__BEAR_JUDGE_GLOBAL_ROOT__ = true; // 锁定全局唯一标识
