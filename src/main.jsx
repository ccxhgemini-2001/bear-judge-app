 ...
 * 修复：移除 new Function，使用安全的解析逻辑，防止 Build Failed
 */
const parseConfig = (val) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("Config parsing failed");
      return null;
    }
  }
};
...
const App = () => {
  ...
  useEffect(() => {
    if (!auth) {
      setError("地基配置异常：未检测到 Firebase 配置。请检查 VITE_FIREBASE_CONFIG 环境变量。");
  ...
  const checkFoundation = () => {
    ...
    console.log("Firebase:", firebaseConfig ? "OK" : "MISSING");
    setError(`自检完成！API 状态：${apiKey ? '就绪' : '缺失'}。`);
  };
  ...
  const triggerAIJudge = async () => {
    ...
    if (!apiKey) { 
      setError("API 密钥未配置，请检查环境变量 VITE_GEMINI_API_KEY"); 
      return; 
    }
    ...
    try {
      ...
      if (err.message === "429") {
        setError("法官大人脑力配额用完啦，进入 60 秒冷静期。");
        setCooldown(60); 
      } else {
        ...
        {!caseId ? (
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-[#F5EBE0] text-center animate-in fade-in zoom-in-95 duration-500 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-[#8D6E63] text-sm mb-12 px-6 font-medium leading-relaxed">这里是王国最神圣的地方嗷，熊将帮你们化解委屈。</p>
              ...
              <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-[#F5EBE0] min-h-[400px] flex flex-col relative overflow-hidden">
                {!userRole && !devMode ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-xs text-[#8D6E63] mb-10 px-10 leading-relaxed">熊在案卷里还没认出你嗷，认领角色后才能提交证词哦！</p>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center py-12 animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-[#8D6E63] text-xs mb-10 px-10 leading-relaxed">{isBothSubmitted ? '双方证词均已归入法典。点击按钮启动正式宣判嗷！' : '正在等待对方提交内心辩词嗷。法庭秩序重于一切～'}</p>
                  ...
                  <div className="mt-16 pt-12 border-t-4 border-double border-[#F5EBE0]">
                    ...
                    <div className="grid grid-cols-1 gap-4 font-bold">
                      {(verdictData.punishments || []).map((p, i) => (
                        <div key={i} className="bg-white border-2 border-[#F5EBE0] p-6 rounded-[2rem] text-center text-sm font-black shadow-sm transition-all hover:translate-y-[-2px] active:border-amber-300 font-bold">{String(p)}</div>
                      ))}
                    </div>
                  </div>
